...
> Slogan: Let's talk about Alg.

!> 转:此文章来源 [github](https://github.com/hustcc/JS-Sorting-Algorithm)
...
![bubbleSort](../media/bubbleSort.gif)
...
![selectionSort](../media/selectionSort.gif)
...
**动图演示**
![insertionSort](../media/insertionSort.gif)
...
def quickSort(arr):
    ...
    left, right = arr[0:pivotIndex], arr[pivotIndex:]
    for i in range(len(arr)):
        if arr[i] < pivot:
            left.append(arr[i])
        else:
...
![countingSort](../media/countingSort.gif)
